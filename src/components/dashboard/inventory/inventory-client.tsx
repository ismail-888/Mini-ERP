...
} from "~/server/actions/product/get-products";
...
export default function InventoryClient({
  ...
  const mappedProducts = useMemo(() => {
    return products.map((p) => ({
      ...
      displayBrand: p.brand?.name ?? "No Brand",
      displayCategory: p.category?.name ?? "Uncategorized",
    }));
  }, [products]);
  ...
  const filteredProducts = mappedProducts.filter((product) => {
    const matchesSearch =
      ...
      product.displayBrand.toLowerCase().includes(searchQuery.toLowerCase());
    ...
    const matchesBrand =
      ...
      selectedBrands.includes(product.brandId ?? ""); // نستخدم الـ ID للمقارنة في الفلتر
  ...
  const stats = {
    ...
    lowStock: mappedProducts.filter(
  ...
  return (
    <div className="">
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        >
          <p className="text-primary text-2xl font-bold">{stats.inStock}</p>
        ...
        <button
          ...
          className={`rounded-lg border p-3 text-left transition-all ${
            stockFilter === "out-of-stock"
              ? "border-destructive bg-destructive/5 ring-destructive ring-1"
              : "border-border bg-card hover:border-muted-foreground"
          }`}
        >
      ...
      <ViewProductDialog
        open={viewOpen}
        productId={viewProductId ?? undefined}
        onClose={() => {
          setViewOpen(false);
