...
export function PosClient({ initialProducts }: PosClientProps) {
  // Use props as initial state. No separate products state needed unless we want to refetch later.
  // For POS, simpler is better: filtering is local.
  const products = initialProducts
  // No loading state needed for products, they are passed in.
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' && searchQuery) {
      ...
      if (product) {
        addItem(product);
        setSearchQuery(""); // Clear search to be ready for next item
        e.preventDefault();
      }
    }
  };
